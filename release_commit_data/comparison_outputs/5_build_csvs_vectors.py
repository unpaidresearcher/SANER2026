...
class Task:
    ...
    def load_pickles(self, project, t1):
        ...
        process_metrics = pickle.load(
            open(os.path.join(pickle_dir, f"{project}_process_metrics_centralities_git_data_vector_norm_fatty.pkl"), "rb"))
        ...
        for file in all_files:
            ...
            for metric_name in PROCESS_METRIC_ORDER:
                vec = process_metrics.get(metric_name, {}).get(t1, {}).get(file, [])
                if vec is None:
                    vec = []
                for idx, val in enumerate(vec, 1):
                    row[f"{metric_name}{idx}"] = val
        ...
        df.to_csv(os.path.join(OUTPUT_DIR, f"{project}_{safe_tag}_vector_fatty_product.csv"), index=False)
