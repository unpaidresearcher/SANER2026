...
def hsiclasso_feature_selection(X_train, y_train, num_features):
    ...
    print(num_features, X_train_np.shape)
    hsic_lasso.input(X_train_np, y_train_np)
    for nf in [40, 30, 20, 10]:
        try:
            print(nf)
            hsic_lasso.classification(min(nf, X_train_np.shape[1]))
            break
        except IndexError:
            print(f"Failed for {nf}, trying smaller...")
...
def performance_calculate(project, bootstrap_samples, selected_features, k):
    train_test_filename = f"{project}_combined_process_vector_fatty_product.csv"
    ...
    result_file1 = open(f"{project}_process_vector_product_predictions_git_data_{k}_fatty.csv", "w")
...
klist = [40]
for k in klist:
    result_file = open(f"process_vector_product_predictions_git_data_{k}_fatty_111class.csv", "w")
    ...
    selected_feature = {}
    for p in projects:
        train_test_filename = f"{p}_combined_process_vector_fatty_product.csv"
        ...
        if k == 0:
            selected_feature_indices = hsiclasso_feature_selection(X_train_test_full_scaled, y_train_test_full,
                                                                   num_features=int(math.log2(len(X_train_test_full))))
        else:
            selected_feature_indices = hsiclasso_feature_selection(X_train_test_full_scaled, y_train_test_full,
                                                                       num_features=k)
