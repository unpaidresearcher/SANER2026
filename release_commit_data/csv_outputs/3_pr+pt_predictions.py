...
def performance_calculate(project, bootstrap_samples, selected_features, k):
    train_test_filename = f"{project}_combined_fatty_product.csv"
    try:
    ...
    result_file1 = open(f"{project}_process_product_metrics_predictions_git_data_{k}_fatty.csv", "w")
    ...
    X_train_test_full, y_train_test_full = train_test_data[[
        ...
        "MaxNesting_Min", "comm", "adev", "ddev", "add", "del", "own", "minor",
                                                            "sctr", "ncomm",
                                                            "nadev", "nddev", "nsctr", "oexp", "exp"]], train_test_data[
                                               'BugPresence']
    ...
    for model in models:
        scores = {'precision': [], 'recall': [], 'accuracy': [], 'f1': [], 'auroc': [], 'brier': [], 'mcc': [], 'auc_pr': []}
        c = 0
        for train_bootstrap, _ in bootstrap_samples[1:]:
...
klist = [40]
for k in klist:
    result_file = open(f"process_product_metrics_predictions_git_data_{k}_fatty_111class.csv", "w")
    ...
    selected_feature = {}
    for p in projects:
        train_test_filename = f"{p}_combined_fatty_product.csv"
        ...
        scaler = MinMaxScaler()
        X_train_test_full_scaled = scaler.fit_transform(X_train_test_full)

        original_columns = ["CountDeclMethodPrivate", "AvgLineCode", "CountLine", "MaxCyclomatic",
            "CountDeclMethodDefault", "AvgEssential", "CountDeclClassVariable",
            "SumCyclomaticStrict", "AvgCyclomatic", "AvgLine", "CountDeclClassMethod",
            "AvgLineComment", "AvgCyclomaticModified", "CountDeclFunction",
            "CountLineComment", "CountDeclClass", "CountDeclMethod",
            ...
            "CountOutput_Max", "CountOutput_Mean", "CountOutput_Min", "CountPath_Max",
            "CountPath_Mean", "CountPath_Min", "MaxNesting_Max", "MaxNesting_Mean",
            "MaxNesting_Min", "comm", "adev", "ddev", "add", "del", "own", "minor", "sctr", "ncomm",
                            "nadev", "nddev", "nsctr", "oexp", "exp"]
      
        imputer = SimpleImputer(strategy='mean')
        X_train_test_full_scaled = imputer.fit_transform(X_train_test_full_scaled)
        print(X_train_test_full_scaled.shape[1])
